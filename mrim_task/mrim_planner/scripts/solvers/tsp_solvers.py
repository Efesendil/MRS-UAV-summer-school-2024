...
class TSPSolver3D():
    ...
    def clusterViewpoints(self, problem, viewpoints, method):
        ...
        if method == 'kmeans':
            ...
            idx = np.array([vp.idx for vp in viewpoints])
            ...
            print("Clustering viewpoints")
            kmeans = KMeans(n_clusters=2, random_state=0, n_init = 10).fit(positions)
            print(f"view point idx: {idx} \n assigned robot:{kmeans.labels_}",)
            
            #print("view points assigned robot:",kmeans.labels_)
