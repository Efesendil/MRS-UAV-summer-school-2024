...
class RRT:
    ...
    def getRandomPointGaussian(self, sigma_offset=0.0):
        if sigma_offset > 20.0:
            print("[INFO] Gaussian sampling with sigma offset {:.2f}.".format(sigma_offset))
        ...
        while not point_valid:
            ...
            p = np.random.normal(loc=mean, scale=sigma+sigma_offset) 
            #sigma*=1.1
            ...
            z = p[2]
            #if(max(sigma)>0.07):
            #    print(f"point {p} sigma {max(sigma+sigma_offset)}")   
            point = Point(x, y, z)
            point_valid = self.pointValid(point)
