...
class TrajectoryUtils():
    ...
    def interpolateHeading(self, waypoints:Viewpoint):
        ...
        while idx != (len(waypoints) - 1):
            ...
            for i in range(1, len(subtraj) - 1):
                ...
                # ask if the interpolation method should be done using subtraj_len
                # get point to point distance
                dst_euc = distEuclidean(subtraj_point_0, subtraj_point_1)
                ratio = dst_euc / subtraj_len

                # [STUDENTS TODO] Change variable 'desired_heading', nothing else
                desired_heading = delta_heading * ratio + current_heading

                # replace heading
                current_heading = wrapAngle(desired_heading)
                wp         = subtraj[i]
    ...
    def sampleStraightSegmentWithStops(self, start, stop):
        ...
        min_ax_vel = min(self.max_velocity[0], self.max_velocity[1], self.max_velocity[2])
        min_ax_acc = min(self.max_acceleration[0], self.max_acceleration[1], self.max_acceleration[2])
    ...
    def resolveCollisions(self, method, problem, trajectories, safety_distance):
        ...
        ## |  [COLLISION AVOIDANCE METHOD #2]: Delay UAV with shorter trajectory at start until there is no collision occurring  |
            ...
            if(trajectories[1].getTime()  > trajectories[0].getTime()):
                # trajectories[1].delayStart(delay_t)
                delay_robot_idx, nondelay_robot_idx = 0, 1
            else:
                # trajectories[0].delayStart(delay_t)
